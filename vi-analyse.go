...
func annotationFlags() client.AnnotationType {
	var flags client.AnnotationType
	if *FlagShotChange {
		flags |= client.ANNOTATION_SHOT_CHANGE
	}
	if *FlagLabel {
		flags |= client.ANNOTATION_LABEL
	}
	if *FlagExplicitContent {
		flags |= client.ANNOTATION_EXPLICIT_CONTENT
	}
	return flags
}

func outputResponseEntity(output *util.Output, t string, label *client.EntityAnnotation) {
...
func outputResponse(status *client.Status, output *util.Output) {
...
func runMain(api *client.VideoIntelligenceService, uris []string) error {
	if len(uris) == 0 {
...
func main() {
	...
	if serviceAccountPath, err := filenameToAbsolute(*FlagServiceAccount); err != nil {
		...
		os.Exit(-1)
	} else if service, err := client.NewVideoIntelligenceServiceFromServiceAccountJSON(serviceAccountPath, *FlagDebug); err != nil {
		fmt.Fprintln(os.Stderr, "Error:", err)
		os.Exit(-1)
